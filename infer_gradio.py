...
# Dummy progress class for API context
class DummyProgress:
    def __init__(self):
        pass
...
@gpu_decorator
def infer(
    ref_audio_orig, ref_text, gen_text, model, remove_silence, cross_fade_duration=0.15, speed=1, show_info=gr.Info, progress=None
):
    ...
    # Use provided progress or create a new one for Gradio context
    if progress is None:
        try:
            progress = gr.Progress()
        except:
            # If gr.Progress() fails (like in API context), use a dummy progress
        ...
        *args,
        ...
        for segment in segments:
            ...
            audio_out, _, ref_text_out = infer(
                ref_audio, ref_text, text, tts_model_choice, remove_silence, 0, show_info=print, progress=DummyProgress()
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Run only the REST API server (without Gradio interface)"
)
def main(port, host, share, api, root_path, api_only):
    global app
    
    # Set default values
    if host is None:
        host = "0.0.0.0" if api_only else "127.0.0.1"
    if port is None:
        port = 8000 if api_only else 7860
    
    if api_only:
        print(f"Starting FastAPI server on {host}:{port}")
        print(f"API documentation available at: http://{host}:{port}/docs")
        print("Available endpoints:")
        print(f"  POST http://{host}:{port}/tts - Text to Speech conversion")
        print(f"  GET  http://{host}:{port}/health - Health check")
        print(f"  GET  http://{host}:{port}/models - Available models")
...
# Create FastAPI app
fastapi_app = FastAPI(title="F5-TTS API", description="REST API for F5-TTS text-to-speech", version="1.0.0")

@fastapi_app.post("/tts", response_model=TTSResponse)
async def text_to_speech_api(
    text: str = Form(...),
    model: str = Form("F5-TTS"),
    ...
    """
    Convert text to speech using F5-TTS or E2-TTS models
    
    Args:
        text: Text to convert to speech
        model: TTS model to use ("F5-TTS" or "E2-TTS")
        remove_silence: Whether to remove silences from output
        cross_fade_duration: Duration of cross-fade between audio clips
        speed: Speed of the generated audio
        ref_text: Reference text (optional, will auto-transcribe if not provided)
    ...
    try:
        ...
        if model not in ["F5-TTS", "E2-TTS"]:
            raise HTTPException(status_code=400, detail="Model must be 'F5-TTS' or 'E2-TTS'")
        
        # Validate and clean text inputs
        ...
        # Generate speech using existing infer function
        audio_output, spectrogram_path, processed_ref_text = infer(
            ref_audio_orig=temp_audio_path,
            ref_text=ref_text,
            gen_text=text,
            model=model,
            ...
            cross_fade_duration=cross_fade_duration,
            speed=speed,
            show_info=lambda x: print(f"Info: {x}"),  # Simple info function for API
            progress=DummyProgress()  # Dummy progress function for API
        )
...
async def health_check():
    ...
    return {"status": "healthy", "message": "F5-TTS API is running"}

@fastapi_app.get("/models")
async def get_available_models():
    """Get list of available TTS models"""
    models = ["F5-TTS"]
    if USING_SPACES or E2TTS_ema_model is not None:
        models.append("E2-TTS")
